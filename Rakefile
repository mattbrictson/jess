...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("jess.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in jess.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    replace_in_file "jess.gemspec", /ruby_version = .*">= (.*)"/ => RubyVersions.lowest
    ...
    replace_in_file "README.md", /requires Ruby (\d\.\d)/i => RubyVersions.lowest
  end
